...
impl CPU {
    ...
    pub fn tick(&mut self, input: [bool; 16], instruction: [bool; 16], reset: bool) {
        ...
        let a_reg_in = gates::mux16(instruction, self.alu_out.0, alu_to_a);
        let load_a = or(a_instruction, alu_to_a);
        self.address_register.tick(a_reg_in, load_a);

        let load_d = and(c_instruction, instruction[4]);
        self.data_register.tick(self.alu_out.0, load_d);

        let (_, zr_out, ng_out) = self.alu_out;
    ...
    pub fn tock(
        ...
        self.a_out = a_out;
        self.alu_out = (out_m, zr_out, ng_out);
