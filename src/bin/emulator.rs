...
fn main() {
    ...
    let binary = emulator::load(&content);
    ...
    let mut rom = emulator::ROM32K::new(&binary);
    let mut cpu = computer::cpu::CPU::new();
    let mut memory = computer::memory::Memory::new();
    ...
    for i in 0.. {
        ...
        for r in 0..256 {
            for c in 0..512 {
                ...
                let content = if is_black { 'X' } else { ' ' };
                t.push(content);
            }
        }
        // write!(stdout, "{}{}", termion::cursor::Goto(r + 1, c + 1), content).unwrap();
        write!(stdout, "{}{} {}", termion::cursor::Goto(1, 1), t.len(), i).unwrap();
        stdout.flush().unwrap();
